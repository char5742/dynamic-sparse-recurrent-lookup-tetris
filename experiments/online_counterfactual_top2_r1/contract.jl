...
function validate_contract(c)
    ...
    c.authorized_implementation_parent_commit == "ddd0f6f83c0e2931cd4cd415531b30b3c80cd2bd" || error("implementation parent changed")
    ...
    c.openvino_backend.full_build == "2026.2.1-21919-ede283a88e3-releases/2026/2" || error("OpenVINO full build changed")
    ...
    c.runtime_source_binding.upstream_tetrisai.repository_relative_path == "upstream/TetrisAI" || error("upstream TetrisAI path changed")
    c.runtime_source_binding.upstream_tetrisai.head == "6fdfb1d30197246fd862b716438e998f0315c830" || error("upstream TetrisAI HEAD changed")
    c.runtime_source_binding.upstream_tetrisai.clean_required === true || error("upstream TetrisAI clean binding disabled")
    c.runtime_source_binding.vendored_tetrisai.node.original_relative_path == "upstream/TetrisAI/src/core/components/node.jl" || error("original node path changed")
    c.runtime_source_binding.vendored_tetrisai.node.vendor_relative_path == "experiments/online_counterfactual_top2_r1/vendor/TetrisAI/src/core/components/node.jl" || error("vendored node path changed")
    c.runtime_source_binding.vendored_tetrisai.node.sha256 == "e98d2052f9248f5c08c1eb58adaace1bd01533f287e682bf35a2fefa1325fe82" || error("vendored node hash changed")
    c.runtime_source_binding.vendored_tetrisai.analyzer.original_relative_path == "upstream/TetrisAI/src/core/analyzer.jl" || error("original analyzer path changed")
    c.runtime_source_binding.vendored_tetrisai.analyzer.vendor_relative_path == "experiments/online_counterfactual_top2_r1/vendor/TetrisAI/src/core/analyzer.jl" || error("vendored analyzer path changed")
    c.runtime_source_binding.vendored_tetrisai.analyzer.sha256 == "24152e2549dcc6c3c25d928454268e8baaa4d45fea31044603917cfbabbe02bc" || error("vendored analyzer hash changed")
    external_runtime = Dict(String(record.path) => (Int(record.bytes), String(record.sha256)) for record in c.runtime_source_binding.external_tetris_runtime)
    external_runtime == Dict(
    ...
    ) || error("external Tetris runtime binding changed")
    c.runtime_source_binding.dependency_graph_digest_encoding == "sorted relative_path + NUL + lowercase sha256 + newline" || error("dependency graph digest encoding changed")
    c.runtime_source_binding.runtime_closure_sha256 == "fa908de68b6deb1581818bdb45c813b06d8886bc4fe33fd010830f7eef03a0e4" || error("runtime closure aggregate changed")
    ...
    c.fit.quantile_method == "linear_type7_position_1_plus_n_minus_1_p" || error("quantile method changed")
