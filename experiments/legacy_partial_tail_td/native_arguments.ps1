...
# Each external P1 phase is placed in a Windows Job Object.  Kill-on-close is
# a last-resort guard if the wrapper itself exits; Terminate() is used for the
# explicit wall, update, and process-tree memory gates.
if (-not ('P1Native.JobObject' -as [type])) {
...
namespace P1Native
{
    public sealed class JobObject : IDisposable
    {
        ...
        public int[] GetProcessIds()
        {
            ...
            // A one-MiB buffer holds more than 130,000 64-bit process IDs,
            // vastly beyond any plausible P1 phase while avoiding a racy
            // query/reallocate pair.
