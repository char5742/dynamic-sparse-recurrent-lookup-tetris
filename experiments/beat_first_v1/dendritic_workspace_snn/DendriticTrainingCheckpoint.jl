module DendriticTrainingCheckpoint
...
if !isdefined(Main, :DendriticArenaTraining)
    Base.include(
        Main,
        joinpath(@__DIR__, "DendriticArenaTraining.jl"),
    )
end

const Core = Main.BeatFirstTrainingCore
const Training = Main.DendriticArenaTraining
const CHECKPOINT_SCHEMA =
    "high-dimensional-dendritic-workspace-snn-checkpoint-v2"

export CHECKPOINT_SCHEMA,
    dendritic_checkpoint_sha256,
    load_dendritic_checkpoint,
    restore_dendritic_checkpoint!,
    save_dendritic_checkpoint
...
function dendritic_checkpoint_sha256(path::AbstractString)
    isfile(path) || error("checkpoint is absent: $path")
    return bytes2hex(SHA.sha256(read(path)))
end

function save_dendritic_checkpoint(
    path::AbstractString,
    trainer,
    sampler,
    run_config;
    ...
    return (;
        path=destination,
        sha256=dendritic_checkpoint_sha256(destination),
        update=Int(update),
    )
end

function load_dendritic_checkpoint(path::AbstractString)
    source = abspath(path)
    ...
    hasproperty(payload, :checkpoint_schema) &&
        ...
        error("unsupported dendritic checkpoint schema")
    return payload
end

function restore_dendritic_checkpoint!(
    trainer,
    payload,
    training_rows,
)
