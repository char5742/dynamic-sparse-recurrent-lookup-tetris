...
if !isdefined(Main, :DendriticWorkspaceSNN)
    Base.include(
        Main,
        joinpath(@__DIR__, "DendriticWorkspaceSNN.jl"),
    )
end
...
const Model = Main.DendriticWorkspaceSNN
...
function refresh_dendritic_cache!(
    ...
    @inbounds for index in eachindex(
        ...
        cache.plateau_threshold[index] = 0.10f0 + probability
    ...
    @inbounds for index in eachindex(parameters.soma_leak_logits)
        ...
        cache.soma_leak[index] = 0.45f0 + 0.50f0 * probability
    ...
    @inbounds for index in eachindex(parameters.apical_gain_logits)
        ...
        cache.apical_gain[index] = 0.75f0 * probability
        cache.apical_gain_derivative[index] =
            0.75f0 * _logistic_derivative(probability)
    end
    @inbounds for index in eachindex(parameters.soma_threshold_logits)
        ...
        cache.soma_threshold[index] = 0.25f0 + 0.75f0 * probability
    ...
    @inbounds for index in eachindex(parameters.adaptation_gain_logits)
        ...
        cache.adaptation_gain[index] = 0.50f0 * probability
...
function dendritic_forward_candidate!(
    ...
    @inbounds for cell in 1:cells
        for branch in 1:model.branches
            exc_rail =
                model.excitatory_feature_for_branch[branch, cell]
            inh_rail =
                model.inhibitory_feature_for_branch[branch, cell]
            seed = muladd(
                parameters.input_exc_gain[branch, cell],
                base.rails[exc_rail, flat],
                parameters.branch_bias[branch, cell],
            ) - parameters.input_inh_gain[branch, cell] *
                base.rails[inh_rail, flat]
            tape.branch_voltage[cell, branch, 1, flat] = seed
    ...
    @inbounds for coordinate in 1:node_dim
        ...
        base.query_pre[coordinate, flat] = value
    ...
    @inbounds for coordinate in 1:node_dim
        base.query[coordinate, flat] = tanh(
    ...
    @inbounds for cycle in 1:model.cycles
        ...
        for cell in 1:cells
            ...
            for branch in 1:model.branches
                seed =
                    tape.branch_voltage[cell, branch, 1, flat]
                drive =
                    scratch.branch_inbox[cell, branch] +
                    0.18f0 * seed
                old_plateau =
                ...
                recruited = max(drive, 0.0f0) * coincidence
                next_plateau = clamp(
                    muladd(
            ...
            tape.cell_spikes[cell, cycle, flat] = active
...
@inline function _accumulate_routing_gradients!(
    ...
    @inbounds for cycle in 1:model.cycles
        for block in 1:model.blocks
            ...
            for coordinate in 1:model.node_dim
                ...
                query = base.query[coordinate, flat]
    ...
    inverse_rms = base.query_inv_rms[flat]
    ...
    @inbounds for coordinate in 1:model.node_dim
        ...
        for rail in 1:InputModel.INPUT_RAILS
            scratch.gradient.query_weight[coordinate, rail] =
                muladd(
                    cotangent,
                    base.rails[rail, flat],
                    scratch.gradient.query_weight[
                        coordinate,
                        rail,
                    ],
                )
        ...
        for cell in 1:cells
            ...
            for branch in 1:model.branches
                ...
                branch_effect =
                    ...
                    plateau_effect
                drive_positive = drive > 0.0f0 ? 1.0f0 : 0.0f0
                recruited_drive_derivative =
                    drive_positive * coincidence +
                    max(drive, 0.0f0) *
                    hard_derivative *
                    cache.plateau_slope[branch, cell]
                combined_drive_effect =
                    branch_effect +
                    plateau_effect *
                    cache.plateau_gain[branch, cell] *
                    recruited_drive_derivative
                ...
                inh_rail =
                    model.inhibitory_feature_for_branch[branch, cell]
                seed_effect =
                    0.18f0 * combined_drive_effect
                scratch.gradient.input_exc_gain[branch, cell] =
                    muladd(
                        seed_effect,
                        base.rails[exc_rail, flat],
                        scratch.gradient.input_exc_gain[
                            branch,
                            cell,
                        ],
                    )
                scratch.gradient.input_inh_gain[branch, cell] =
                    muladd(
                        -seed_effect,
                        base.rails[inh_rail, flat],
                        scratch.gradient.input_inh_gain[
                            branch,
                            cell,
                        ],
                    )
...
@inline function _update_edge_trace(
    ...
    next_u = muladd(
        cache.branch_leak[branch, destination],
        epsilon_u,
        forcing +
    ...
    coincidence_derivative =
        ...
        cache.plateau_slope[branch, destination] *
        next_u
    recruited_derivative =
        branch_drive > 0.0f0 ?
        forcing * coincidence +
        branch_drive * coincidence_derivative : 0.0f0
    next_p = muladd(
    ...
    next_s *=
        1.0f0 -
        cache.soma_threshold[destination] * post_surrogate
    return next_u, next_p, next_s
end
    ...
    @inbounds for cycle in 1:model.cycles
        ...
        for active_index in 1:scratch.active_edge_count
            ...
            branch_drive =
                ...
                cache.branch_leak[branch, destination] *
                old_branch -
                cache.plateau_feedback[branch, destination] *
                old_plateau
            next_apical =
    ...
    @inbounds for relation in 1:model.fanout
        for source in axes(branch_for_edge, 1)
            branch_for_edge[source, relation] =
                UInt8(mod1(source + 3relation, model.branches))
        end
    end
    gate_mask = BitMatrix(
...
function _reduce_worker_accumulators!(
    ...
    _reduce_gradient_field!(trainer, workers, Val(:input_exc_gain), recurrent)
    _reduce_gradient_field!(trainer, workers, Val(:input_inh_gain), recurrent)
    _reduce_gradient_field!(trainer, workers, Val(:branch_bias), recurrent)
    _reduce_gradient_field!(trainer, workers, Val(:branch_leak_logits), recurrent)
    _reduce_gradient_field!(trainer, workers, Val(:plateau_decay_logits), recurrent)
    _reduce_gradient_field!(trainer, workers, Val(:plateau_threshold_logits), recurrent)
    _reduce_gradient_field!(trainer, workers, Val(:plateau_slope_logits), recurrent)
    _reduce_gradient_field!(trainer, workers, Val(:plateau_gain_logits), recurrent)
    _reduce_gradient_field!(trainer, workers, Val(:plateau_feedback_logits), recurrent)
    _reduce_gradient_field!(trainer, workers, Val(:soma_coupling), recurrent)
    _reduce_gradient_field!(trainer, workers, Val(:apical_leak_logits), recurrent)
    _reduce_gradient_field!(trainer, workers, Val(:soma_leak_logits), recurrent)
    _reduce_gradient_field!(trainer, workers, Val(:adaptation_decay_logits), recurrent)
    _reduce_gradient_field!(trainer, workers, Val(:apical_gain_logits), recurrent)
    _reduce_gradient_field!(trainer, workers, Val(:soma_threshold_logits), recurrent)
    _reduce_gradient_field!(trainer, workers, Val(:adaptation_gain_logits), recurrent)
    _reduce_gradient_field!(trainer, workers, Val(:query_weight), recurrent)
    _reduce_gradient_field!(trainer, workers, Val(:workspace_key), recurrent)
    _reduce_gradient_field!(trainer, workers, Val(:feedback_gain), recurrent)
    _reduce_gradient_field!(trainer, workers, Val(:synapse_weight), recurrent)
    _reduce_gradient_field!(trainer, workers, Val(:gate_logits), recurrent)
    _reduce_gradient_field!(trainer, workers, Val(:delay_logits), recurrent)
    _reduce_gradient_field!(trainer, workers, Val(:workspace_decay_logit), recurrent)
    _reduce_gradient_field!(trainer, workers, Val(:head_weight), 1.0f0)
    _reduce_gradient_field!(trainer, workers, Val(:head_bias), 1.0f0)
    _reduce_gradient_field!(trainer, workers, Val(:output_weight), 1.0f0)
    _reduce_gradient_field!(trainer, workers, Val(:output_bias), 1.0f0)
    trainer.metrics.gradient_norm = _tree_norm(trainer.gradient)
    maximum_norm = 5.0
...
function _adam_shard!(
    ...
    norm_square = if field == 1
        _adam_range!(trainer, Val(:input_exc_gain), first_index, last_index)
    elseif field == 2
        _adam_range!(trainer, Val(:input_inh_gain), first_index, last_index)
    elseif field == 3
    ...
    elseif field == 17
        _adam_range!(trainer, Val(:query_weight), first_index, last_index)
    elseif field == 18
...
function dendritic_arena_update!(
    ...
    local_seconds =
        signal_seconds + local_replay_seconds
