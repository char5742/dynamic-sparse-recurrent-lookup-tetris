...
## 65,000更新から100,000更新までの本学習

速度条件に合格したscale `1.0`の65k checkpointから、同じ構成を100kまで継続した。
モデル、入力、teacher、task loss、optimizer semantics、hard halting、1-step probe、
schedulerは変更していない。

| 更新 | held loss | held top-1 | held NDCG | held pairwise | held margin | held平均深度 | 深度範囲 | 累積区間updates/s |
|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 65,000 | 2.605426 | 0.742188 | 0.990088 | 0.893202 | 0.140937 | 3.217 | 2～12 | - |
...
35,000更新の実時間は1,803.908秒、`19.402 updates/s`、`77.609 states/s`、
CPU平均`59.245%`、candidate中`60.224%`だった。最低15 updates/sを全区間で維持した。

95kは65k比でloss `-0.019530`、NDCG `+0.001191`、pairwise `+0.008762`、
margin `+0.016603`、top-1同値だった。100kではloss `+0.015532`、top-1
`-0.015625`、NDCG `-0.001564`、pairwise `-0.004749`と95kから反落した。
このため、100kを機械的に採用せず、95kを速度条件付きの連続順位最良点とする。

top-1の最良点は85k、pairwiseの最良点は90kであり、単一指標の最良更新は一致しない。
平均深度も2.01～3.02を往復した。90～100kは学習不能ではないが、安定した単調改善ではなく、
小規模固定panel上で振動を伴う頭打ち域である。

```text
run: evrl_boundsfix_p2_c0_halt5e5_lr2e4_full65k_wd3e4_u100000_20260723_long1
adopted checkpoint: checkpoint_000095000.jls
sha256: 622753d65e0502edd09ed4ef10ecf3270b90e7a92e7d9b2d44cb6d71d9350893
teacher states: 380,000
final checkpoint: checkpoint_000100000.jls
final sha256: 6ebb042a3678d9445ed76499730be15e63527b716c7c77322b2e8fca00cc4b45
status: complete
```

### PreActとの予算別比較

同じ固定128状態panelを使うが、反復観測済みであるため独立汎化評価ではない。

- sample efficiency：PreAct 12k、48,000 statesはtop-1 `0.789062`、NDCG
  `0.993292`、loss `2.563784`。EVRLは10k、40,000 statesでも15k、60,000 statesでも
  これに届かなかった。
- ほぼ同じwall-clock：EVRL 95kは5,817秒。PreAct 3kは約6,130秒で、top-1
  `0.703125`、NDCG `0.989252`、pairwise `0.893250`、loss `2.715674`だった。
  EVRL 95kは同じ順に`0.742188`、`0.991278`、`0.901964`、`2.585896`で上回った。
- 到達品質：PreAct 12.75kはtop-1 `0.789062`、NDCG `0.994468`、pairwise
  `0.930203`、loss `2.550905`で、EVRL 95kより高い。EVRLはmarginのみ高い。

したがって、EVRLはCPU wall-clock当たりの学習量と同時間品質で優位だが、teacher state当たりの
sample efficiencyと現時点の最高到達品質ではPreActを超えていない。
