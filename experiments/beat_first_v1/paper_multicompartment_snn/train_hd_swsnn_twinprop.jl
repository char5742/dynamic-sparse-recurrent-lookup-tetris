...
function usage()
    ...
    julia --threads=N,0 train_hd_swsnn_twinprop.jl [options]
    ...
    Runs longer than 64 updates require --cell-mode distilled. Detailed mode
    is an explicit mechanism control only. This command has no resume mode
    and rejects existing output directories.
    """
end
...
function parse_options(arguments)
    ...
    while index <= length(arguments)
        ...
        name == "resume" && error(
            "HD-SWSNN-TwinProp training is scratch-only; --resume is forbidden",
        )
...
function internal_audit(trainer)
    return (;
        ...
        max_delta=Float64(
            paper_internal_max_delta(trainer),
        ),
    )
end
...
function verify_frozen_internal!(
    ...
    current = internal_audit(trainer)
    current == initial_internal ||
        error("$label changed the frozen internal cell")
    if cell_mode == "distilled-frozen"
        current.artifact_sha256 ==
            lowercase(
                artifact_metadata.distilled_artifact_hash,
            ) || error("$label artifact hash differs")
        current.parameter_sha256 ==
            lowercase(
                artifact_metadata.internal_parameter_sha256,
            ) || error("$label in-memory parameter hash differs")
    end
    current.max_delta == 0.0 ||
        error("$label changed frozen internal parameters")
    return current
end

function save_training_checkpoint!(
    trainer,
    sampler,
    run_config,
    checkpoint_dir,
    ...
    record.internal_artifact_sha256 ==
        audit.artifact_sha256 ||
        error("checkpoint recorded a different artifact hash")
    record.internal_parameter_sha256 ==
        audit.parameter_sha256 ||
        error("checkpoint recorded a different parameter hash")
    println(
        manifest_io,
    ...
    println(
        ...
        "$(record.internal_parameter_sha256) path=$(record.path)",
    )
    flush(stdout)
    return record
end
