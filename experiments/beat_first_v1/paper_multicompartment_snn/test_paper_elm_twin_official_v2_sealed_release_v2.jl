...
using .PaperELMTwinOfficialV2SealedReleaseV2

const Sealed = PaperELMTwinOfficialV2SealedReleaseV2
const DEV1500_ROOT =
...
@testset "Paper ELM v2 sealed release boundary" begin
    ...
    @test Sealed ===
        Main.PAPER_ELM_OFFICIAL_V2_SEALED_RELEASE_CANONICAL
    @test Sealed.Twin ===
        Main.PAPER_ELM_OFFICIAL_V2_PROFILED_CANONICAL_V3
    @test Sealed.Twin.ProfiledOfficialPaperELMTwin !== nothing
    ...
    if isfile(DEV1500_MANIFEST)
        @testset "sealed dev1500 source and splits" begin
            ...
            @test all(dataset.records) do record
                !(
                    Sealed._record_overlaps_ids(record, dataset.fit_ids) &&
                    Sealed._record_overlaps_ids(record, dataset.heldout_ids)
                )
            end
        ...
        @testset "shard-byte tamper is rejected" begin
            mktempdir() do directory
                ...
                @test_throws ErrorException Sealed._verify_manifest_and_shards(
