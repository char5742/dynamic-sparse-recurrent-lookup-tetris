...
const PinnedV5ProductionV2 =
    Main.HD_SWSNN_TWINPROP_PINNED_V5_CANONICAL_FINAL
const PinnedV5ArenaV2 = PinnedV5ProductionV2.Training
const PinnedV5ModelV2 = Main.PaperModelCanonical
...
elseif Main.PaperArenaTraining !== PinnedV5ArenaV2
...
const PINNED_V2_RUNNER_SCHEMA =
    "hd-swsnn-twinprop-pinned-v5-checkpoint-lineage-v2"
const PINNED_V2_MODEL_SEED = UInt64(0x48445357534e4e4d)
const PINNED_V2_SAMPLER_SEED = UInt64(0x48445357534e4e53)
const PINNED_V2_ROUTING_SEED = UInt64(0x48445357534e4e52)
const PINNED_V2_WIDTH = 208
const PINNED_V2_OPTIMIZER_GROUPS = Set((
    :input_conductance,
...
function _pinned_v2_file_sha256(path)
    isfile(path) || error("file is absent: $path")
    return bytes2hex(SHA.sha256(read(path)))
end

function _pinned_v2_dataset_manifest_sha256(path)
    manifest = joinpath(path, "manifest.json")
    isfile(manifest) ||
        error("Tetris dataset manifest is absent: $manifest")
    return _pinned_v2_file_sha256(manifest)
end

function _pinned_v2_source_revision()
    try
...
function _pinned_v2_write_json(path, value)
    open(path, "w") do io
...
function _pinned_v2_sidecar_path(checkpoint_path, suffix)
    stem, _ = splitext(checkpoint_path)
    return stem * suffix
end

function _pinned_v2_periodic_path(checkpoint_path, update)
...
function _pinned_v2_lineage_id(
    options,
    artifact,
    dataset_manifest_sha256,
)
    fields = (
        PINNED_V2_RUNNER_SCHEMA,
        MODEL_FAMILY,
        ...
        dataset_manifest_sha256,
        string(PINNED_V2_MODEL_SEED),
        string(PINNED_V2_SAMPLER_SEED),
        string(PINNED_V2_ROUTING_SEED),
        string(PINNED_V2_WIDTH),
        ...
        string(options.cpuset_mode),
        string(options.stochastic_routing),
    )
    return bytes2hex(SHA.sha256(codeunits(join(fields, '\n'))))
end

function _pinned_v2_validate_paths!(options)
...
function _pinned_v2_run_config(
    options,
    artifact,
    dataset_manifest_sha256,
    lineage_id,
    ...
    return (;
        ...
        dataset_manifest_sha256,
        model_seed=string(PINNED_V2_MODEL_SEED),
        sampler_seed=string(PINNED_V2_SAMPLER_SEED),
        routing_seed=string(PINNED_V2_ROUTING_SEED),
        ...
        source_revision=_pinned_v2_source_revision(),
...
function _pinned_v2_assert_input_lineage!(
    payload,
    lineage_id,
    target_updates,
)
    hasproperty(payload.run_config, :runner_schema) &&
        payload.run_config.runner_schema == PINNED_V2_RUNNER_SCHEMA ||
        error("checkpoint was not created by the pinned-V5 V2 runner")
    ...
    hasproperty(payload.run_config, :lineage_origin) &&
        payload.run_config.lineage_origin == "scratch" ||
        error("checkpoint does not descend from a scratch lineage")
    ...
    target_updates == 1_000 &&
        !(64 <= start_update < 1_000) &&
        error(
            "the 1k stage must descend from update 64 " *
            "or an in-stage checkpoint",
        )
    target_updates == 10_000 &&
        !(1_000 <= start_update < 10_000) &&
        error(
            "the 10k stage must descend from update 1000 " *
            "or an in-stage checkpoint",
        )
    return start_update
end

function _pinned_v2_save_checkpoint!(
    path,
    trainer,
    sampler,
    run_config,
    ...
    PinnedV5ArenaV2.paper_checkpoint_integrity!(trainer)
...
function pinned_v5_v2_main(arguments=ARGS)
    options = parse_pinned_v5_options(arguments)
    if options.help
        print(pinned_v5_v2_usage())
        return nothing
    end
    _pinned_v2_validate_paths!(options)
    Threads.nthreads(:interactive) == 0 ||
        error("launch with --threads=N,0")
    BLAS.set_num_threads(1)

    bundle =
        PinnedV5ProductionV2.load_pinned_v5_production_bundle(
    ...
    PinnedV5ProductionV2.assert_pinned_v5_bundle_unchanged!(
        bundle,
    )
    ...
    dataset = load_teacher_dataset(
        options.dataset;
        max_candidates=PINNED_V2_WIDTH,
        allow_partial_dataset=false,
        geometry_cache_max_states=1,
    )
    maximum(dataset.action_counts) <= PINNED_V2_WIDTH ||
        error("teacher candidates exceed width $PINNED_V2_WIDTH")
    rows = Int.(findall(==(:train), dataset.predefined_split))
    isempty(rows) && error("Tetris dataset has no train split")
    dataset_manifest_sha256 =
        _pinned_v2_dataset_manifest_sha256(options.dataset)

    model = PinnedV5ModelV2.build_paper_model(:paper_scaled_v1)
    parameters, _ = Lux.setup(
        Xoshiro(PINNED_V2_MODEL_SEED),
        model,
    )
    builder_options = (;
        state_batch=1,
    ...
    trainer = if options.require_production
        PinnedV5ProductionV2.build_production_trainer(
            bundle,
            model,
            parameters;
            builder_options...,
        ...
        PinnedV5ProductionV2.build_development_trainer(
            bundle,
            model,
            parameters;
    ...
    aux = PinnedV5ArenaV2.register_paper_trainer_aux!(trainer)
    aux isa PinnedV5ArenaV2.PaperReleaseAux ||
    ...
    PinnedV5ArenaV2.paper_preflight_integrity!(trainer)
    initial_parameter_hash =
        PinnedV5ArenaV2.paper_internal_parameter_sha256(trainer)
    initial_artifact_hash =
        PinnedV5ArenaV2.paper_internal_sha256(trainer)
    initial_parameter_hash ==
        bundle.distilled_parameter_sha256 ||
    ...
    PinnedV5ArenaV2.paper_internal_max_delta(trainer) == 0.0f0 ||
        error("frozen internal cell changed before training")

    lineage_id = _pinned_v2_lineage_id(
        options,
        artifact,
        dataset_manifest_sha256,
    )
    ...
    start_update = options.scratch ? 0 :
        _pinned_v2_assert_input_lineage!(
            payload,
            lineage_id,
            options.updates,
        )
    ...
    run_config = _pinned_v2_run_config(
        options,
        artifact,
        dataset_manifest_sha256,
        lineage_id,
    ...
    sampler = if options.scratch
        EpochSampler(rows, Xoshiro(PINNED_V2_SAMPLER_SEED))
    else
        restore_checkpoint!(
            trainer,
            payload,
    ...
    executor = PinnedV5ArenaV2.PaperExecutorFinal(
        trainer,
        dataset;
        active_workers=options.workers,
        stochastic_routing=options.stochastic_routing,
        routing_seed=PINNED_V2_ROUTING_SEED,
        cpuset_mode=options.cpuset_mode,
    )
    all(
        worker -> worker.runtime isa
            PinnedV5ArenaV2.ReleaseCellRuntime,
        executor.workers,
    ) || error("final executor contains a non-release runtime")

    mkpath(dirname(options.checkpoint_out))
    trace_path = _pinned_v2_sidecar_path(
        options.checkpoint_out,
        ".trace.tsv",
    )
    manifest_path = _pinned_v2_sidecar_path(
        options.checkpoint_out,
        ".manifest.tsv",
    )
    result_path = _pinned_v2_sidecar_path(
        options.checkpoint_out,
        ".result.json",
    )
    _pinned_v2_write_json(
        _pinned_v2_sidecar_path(
            options.checkpoint_out,
            ".config.json",
        ),
    ...
    open(trace_path, "w") do trace_io
        ...
        open(manifest_path, "w") do manifest_io
            ...
            PinnedV5ArenaV2.run_with_paper_team!(executor) do running
                for update in (start_update + 1):options.updates
                    PinnedV5ArenaV2.Point.fill_next_rows!(
                        PinnedV5ArenaV2.paper_training_arena(
                            trainer,
                        ).rows,
                        sampler,
                    )
                    PinnedV5ArenaV2.paper_arena_update!(running)
                    ...
                    if update < options.updates &&
                       update % options.checkpoint_interval == 0
                        last_checkpoint =
                            _pinned_v2_save_checkpoint!(
                                _pinned_v2_periodic_path(
                                    options.checkpoint_out,
                                    update,
                                ),
                                trainer,
            ...
            last_checkpoint = _pinned_v2_save_checkpoint!(
                options.checkpoint_out,
                trainer,
                sampler,
                run_config,
    ...
    final_parameter_hash =
        PinnedV5ArenaV2.paper_end_run_integrity!(trainer)
    ...
    artifact_after =
        cell_artifact_metadata(options.distilled_cell)
    ...
    result = (;
        schema=
            "hd-swsnn-twinprop-pinned-v5-segment-result-v2",
        completed=true,
        model_family=MODEL_FAMILY,
        lineage_id,
        lineage_origin="scratch",
        segment_start_mode=
            options.scratch ? "scratch" : "checkpoint",
        start_update,
        ...
        frozen_internal_audit=(;
            ...
            internal_max_delta=
                PinnedV5ArenaV2.paper_internal_max_delta(trainer),
    ...
    _pinned_v2_write_json(result_path, result)
    println(
