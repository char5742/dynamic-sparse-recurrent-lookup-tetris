# Included into `PaperELMTwinOfficialV2SealedReleaseV2`.
# This overlay changes only held-out measurement semantics. Gate thresholds and
# the one-shot held-out audit remain those of sealed V2.

const SEALED_V2_CONTRACT_FIX_V2_APPLIED = true
const _SEALED_V2_CONTRACT_FIX_V2_SOURCE = @__FILE__
const _SEALED_V2_CONTRACT_FIX_V2_EVALUATOR_ID =
    "official-final-v2-signed1278-paper-window-reset-exact-auroc-v4-contract-fix"
const _SEALED_V2_NMDA_SCALE_FLOOR_V2 = 1.0e-5

corrected_evaluator_source_sha256_v2() =
    _file_sha256(_SEALED_V2_CONTRACT_FIX_V2_SOURCE)

_clip_official_soma_target_v2(target) =
    min.(target, Twin.OFFICIAL_SOMA_CLIP_MV)

function _contract_normalized_rmse_v2(
    ...
    if centered > eps(Float64)
        # This is exactly the pre-existing non-zero-variance formula.
        return sqrt(moments.error2 / centered)
    end
    scale = max(
        abs(Float64(frozen_nmda_scale)),
        _SEALED_V2_NMDA_SCALE_FLOOR_V2,
    )
    ...
    for record in dataset.records
        ...
        for (item, id) in enumerate(ids)
            ...
            for (window_index, start_step) in
                ...
                target_voltage = _clip_official_soma_target_v2(
...
function _contract_fixed_evaluator_v2(base)
    return merge(
        base,
        (;
            id=_SEALED_V2_CONTRACT_FIX_V2_EVALUATOR_ID,
            source_sha256=corrected_evaluator_source_sha256_v2(),
...
function attest_sealed_official_elm_release(
    ...
    payload = merge(
        base_payload,
        (; evaluator=_contract_fixed_evaluator_v2(base_payload.evaluator)),
    )
