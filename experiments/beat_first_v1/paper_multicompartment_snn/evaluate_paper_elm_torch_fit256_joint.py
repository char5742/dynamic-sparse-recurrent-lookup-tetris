"""Corrected fit256 derived-validation joint metrics (heldout forbidden)."""
...
import run_paper_elm_torch_cpu_fit256_balanced_jit as entry
        ...
        for trial_id in core.VALIDATION_IDS:
            ...
            for window_index, start_julia in enumerate(starts_julia):
                ...
                voltage_error2 += float(
                    np.square(predicted_mv - clipped_target_mv).sum()
                )

                predicted_nmda = selected[:, 2:].double().cpu().numpy()
                target_coordinate = (
...
def main() -> None:
    ...
    bridge = runner._small_bridge(full_bridge, 256)
    model = runner._build(bridge, 256, "torchscript")
