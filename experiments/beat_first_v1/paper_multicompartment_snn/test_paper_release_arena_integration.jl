...
function require_release_test_inputs()
    isfile(RELEASE_TEST_ARTIFACT) || error(
        "FAIL-CLOSED: genuine release-v2 artifact is absent: " *
        RELEASE_TEST_ARTIFACT,
    )
    isdir(RELEASE_TEST_DATASET) || error(
        "real teacher_v3 dataset is absent: " *
        RELEASE_TEST_DATASET,
    )
    return nothing
end

function make_release_trainer()
    ...
    trainer = PaperTrainer(
        ...
        cell_artifact=RELEASE_TEST_ARTIFACT,
    )
...
function run_release_integration_test()
    require_release_test_inputs()
    BLAS.set_num_threads(1)
    trainer, aux = make_release_trainer()

    @testset "release contract" begin
        @test aux isa PaperArenaTrainingFinal.PaperReleaseAux
        @test eltype(aux.location_catalog) === UInt16
        @test length(aux.location_catalog) == 642
        @test first(aux.location_catalog) == UInt16(1)
        @test last(aux.location_catalog) == UInt16(642)
        ...
        @test PaperArenaTrainingFinal.ReleaseCell.
    ...
    worker = PaperArenaTrainingFinal.PaperWorker(trainer)
    @testset "trusted hard-spike hot path" begin
        @test worker.runtime isa
            PaperArenaTrainingFinal.ReleaseCellRuntime
        runtime = worker.runtime
        reset_runtime!(runtime)
        reset_cell_drive!(runtime, 1)
        add_cell_event!(
            runtime,
            1,
            642,
        ...
        step_cell!(runtime, 1)
        reset_cell_drive!(runtime, 1)
        allocated = @allocated step_cell!(runtime, 1)
        @test allocated == 0
