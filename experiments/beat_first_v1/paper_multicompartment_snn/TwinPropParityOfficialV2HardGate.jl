"""
Fail-closed post-projection gate for the official 1,278-channel TwinProp path.

The public preprint does not disclose the differentiable parameterization used
for discrete synaptic locations.  This project reconstruction optimizes a soft
location distribution and then projects every axon to exactly
`contacts_per_axon` anatomical contacts.  A soft score is therefore never
accepted as evidence of XOR/parity performance.

Every restart is replayed through the frozen digital twin after the exact hard
projection.  Restart selection uses the hard replay, and NEURON transfer is
permitted only when the fixed soft-to-hard accuracy and BCE gates pass.
"""
...
@eval TwinPropParityOfficial.TwinPropParity begin
    ...
    """
    Construct the exact signed 1,278-channel ELM input produced by the hard
    anatomical contact map.

    The first 639 channels are excitatory events on Hay segments 2:640 and the
    second 639 are inhibitory events with negative sign.  Repeated contacts are
    distinct one-micrometre sites and therefore contribute multiplicity.
    """
    ...
    function _decision_metrics_from_twin_output(
        output,
    ...
    """
    Replay one exact hard contact map through the frozen official ELM.

    This is a digital-twin metric, not the final detailed-NEURON result.  It is
    solely the mandatory projection-fidelity gate before detailed transfer.
    """
    function hard_projection_replay_metrics(
        ...
        metrics = _decision_metrics_from_twin_output(output, dataset)
        return merge(
            metrics,
            (
                input_contract=
    ...
    """
    Record soft and exact-hard frozen-twin metrics and their projection gap.
    """
    ...
    """
    Optimize each restart independently, project it to exact contacts, replay
    train/test/clean through the frozen twin, and select only among hard-gate
    passing restarts.

    If every restart suffers a projection cliff, this function fails before
    NEURON transfer.  No soft-only score can be promoted.
    """
    function train_twinprop_hard_gated(
        ...
        return (
            ...
            gate=(
                ...
                selected_by="hard_frozen_twin_accuracy_minus_1e-3_bce",
                thresholds=(
                    max_accuracy_drop=
