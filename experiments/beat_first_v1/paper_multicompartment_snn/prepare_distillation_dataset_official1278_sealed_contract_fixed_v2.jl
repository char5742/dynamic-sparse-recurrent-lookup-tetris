module DistillationDatasetBridgeOfficial1278SealedContractFixedV2

"""
Critical-path adapter from the corrected sealed V2 artifact to the canonical
official-1278 bit-exact writer.

The adapter never deserializes teacher shards while extracting the embedded
frozen twin.  It validates the sealed container, corrected evaluator identity,
fixed gate, frozen hashes, and source-manifest identity; writes the exact
canonical bare-frozen format atomically; then delegates all source verification
and post-write bit-exact replay to the existing final-V2 writer.
"""
...
gate, and every cached primary target before it publishes a bit-exact dataset.
"""
...
end # module DistillationDatasetBridgeOfficial1278SealedContractFixedV2

if abspath(PROGRAM_FILE) == @__FILE__
    DistillationDatasetBridgeOfficial1278SealedContractFixedV2.main()
end
