module StreamingOfficialELMReleaseDatasetV4

# Standalone consumer for the canonical final.v2 sealed ELM.  It intentionally
# does not include or alias the superseded final.v1 reader/type.
...
const Sealed =
    getfield(_PARENT, :PaperELMTwinOfficialV2SealedReleaseV2)
...
function _required(object, name::Symbol)
    ...
    value === nothing &&
        error("sealed-v2 stream field $(String(name)) is absent")
    return value
end
...
function _assert_time_contract(manifest, time_steps)
    ...
    full_time_steps == time_steps ||
        error("sealed-v2 bridge did not preserve the full trajectory")
    ...
    full_time_steps > input_window_steps + 500 ||
        error("trajectory has no valid Python-exclusive random window")
    starts = _tuple2(
        _required(
            training,
...
function _assert_input_contract(manifest)
    ...
    String(_required(layout, :input_to_synapse_routing)) ==
        "neuronio_routing" ||
        error("official ELM routing differs")
    return true
end
...
function _assert_primary_claim(manifest)
    ...
    _required(claim, :required) === true ||
        error("bridge does not require live primary verification")
    _required(claim, :all_samples) === true ||
        error("bridge did not live-check all cached samples")
    _required(claim, :bit_exact) === true ||
        error("bridge primary cache is not bit exact")
    Tuple(String.(collect(_required(claim, :targets)))) == (
        "soma_voltage",
        "spike_probability",
        "spike_logit",
        "regional_nmda_current",
    ) || error("primary target identity/order differs")
...
function _verify_bundle_and_manifest!(
    ...
    String(_required(manifest, :digital_twin_type)) ==
        SEALED_EXECUTION_TYPE ||
        error("bridge did not consume the exact final.v2 sealed type")
    ...
    if require_production
        ...
        payload.outcome.promotable_production === false ||
            error("development bridge must remain non-promotable")
    ...
    provenance = merge(
        ...
        (;
            ...
            source_model_parameter_sha256=
                bundle.attestation.payload.model.parameter_sha256,
            source_model_base_artifact_sha256=
                bundle.attestation.payload.model.base_artifact_sha256,
            executable_mlp_activation=
                bundle.attestation.payload.model.
                    executable_mlp_activation,
            compatibility_profile=
                bundle.attestation.payload.model.compatibility_profile,
            time_contract,
        ),
    )
...
function _validate_logit_target(shard, time_steps, samples)
    ...
    size(logit) == (time_steps, samples) ||
        error("cached spike-logit shape differs")
    all(isfinite, logit) ||
        error("cached spike logits are non-finite")
    return logit
end
...
function sealed_stream_materialize_window(
    ...
    dataset.input_dim == OFFICIAL_ELM_INPUT_DIM ||
        error("dataset is not a sealed-v2 1278-input stream")
...
function _bit_exact(name, live, cached)
    ...
    reinterpret(UInt32, vec(Float32.(live))) ==
        reinterpret(UInt32, vec(Float32.(cached))) ||
        error("$name cache is not bit-identical to the live sealed-v2 ELM")
    return 0.0
end
    ...
    for (shard_index, record) in enumerate(dataset.records)
        ...
        for local_index in 1:record.samples
            ...
            for first_time in 1:chunk:dataset.time_steps
                last_time =
                    min(first_time + chunk - 1, dataset.time_steps)
                ...
                maxima[1] = max(maxima[1], _bit_exact(
                    "soma voltage",
                    live.voltage,
                ...
                maxima[2] = max(maxima[2], _bit_exact(
                    "spike probability",
                ...
                maxima[3] = max(maxima[3], _bit_exact(
                    "spike logit",
                    live.spike_logit,
                    @view(logit[
                        first_time:last_time,
                ...
                maxima[4] = max(maxima[4], _bit_exact(
...
end # module StreamingOfficialELMReleaseDatasetV4
