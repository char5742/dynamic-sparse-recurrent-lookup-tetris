module DistilledElevenStateCellReleaseRuntimeV3
...
function load_release_runtime(path::AbstractString)
    ...
    _get(payload, :training_input_mode, "") ==
        ...
        error("release artifact did not use the canonical sharded consumer")
    _get(payload, :dense_full_memory_path, true) === false ||
        error("dense/full-memory distillation cannot be promoted")
    _get(payload, :provisional, true) === false ||
        error("provisional distilled artifacts cannot be loaded")
    String(_required(payload, :prepared_dataset_schema)) ==
        STREAM_DATASET_SCHEMA ||
        error("release artifact did not consume the final v2 shards")
    paper_scale = _required(payload, :paper_scale)
    paper_scale isa Bool ||
        error("release artifact does not disclose paper_scale")
    ...
    Float64(_get(test, :spike_auroc, NaN)) >=
        MINIMUM_SPIKE_AUROC ||
        error("release held-out conservative spike AUROC is below 0.985")
    _get(test, :auroc_is_conservative_lower_bound, false) === true ||
        error("streaming AUROC is not a conservative lower bound")
    ...
    _get(contract, :semantic_state_scale, "") ==
        "normalized_unit_interval" ||
        error("release semantic state scale differs")
    _get(contract, :location_index_type, "") == "UInt16" ||
        error("release location index type differs")
...
end # module DistilledElevenStateCellReleaseRuntimeV3
