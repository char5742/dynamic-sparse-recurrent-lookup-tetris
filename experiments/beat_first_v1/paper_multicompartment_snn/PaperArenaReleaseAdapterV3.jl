# Canonical release-v3 adapter for the exact FinalProduction arena.
#
# This file must be included directly into
# Main.PaperArenaTrainingFinalProduction after that module aliases the one
# canonical DistilledElevenStateCellFinal type. It deliberately does not include
# or wrap the legacy V2 adapter.
#
# Production contract:
#   * DistilledElevenStateCellReleaseRuntimeV3.TrustedReleaseRuntime, including
#     the disclosed paper_scale field;
#   * paper_scale=true for the production paper arm;
#   * all 642 official Hay segment IDs retained for morphology/projection/hash
#     identity and utility indexing;
#   * only the 639 dendritic IDs 2:640 are legal synaptic contact targets;
#   * normalized semantic 11-state representation and frozen trusted step;
#   * integrity hashes at preflight/checkpoint/run-end boundaries.
#
# V3 load_release_runtime remains the single authority that rejects old V2,
# provisional, dense/full-memory, failed-gate, and mapping-mismatched artifacts.

isdefined(@__MODULE__, :ReleaseCell) &&
    error("a legacy release adapter is already installed in this arena")
isdefined(@__MODULE__, :DistilledElevenStateCellFinal) ||
    error("FinalProduction must alias the canonical Final cell before V3 adapter load")
include(joinpath(@__DIR__, "DistilledElevenStateCellReleaseRuntimeV3.jl"))
const ReleaseCell = DistilledElevenStateCellReleaseRuntimeV3
const RELEASE_ADAPTER_CONTRACT_VERSION = 3
...
_release_legal_location_catalog() =
    UInt16.(RELEASE_LEGAL_CONTACT_RANGE)
...
function _assert_release_legal_contact_state!(
    ...
    aux.location_catalog == _release_legal_location_catalog() ||
        error("release contact catalog is not exactly UInt16 IDs 2:640")
    ...
    size(aux.input_location_utility, 1) ==
        ...
        error("input utility no longer preserves 642 official indices")
    size(aux.recurrent_location_utility, 1) ==
        RELEASE_OFFICIAL_SEGMENT_COUNT ||
        error("recurrent utility no longer preserves 642 official indices")
    size(aux.workspace_location_utility, 1) ==
        RELEASE_OFFICIAL_SEGMENT_COUNT ||
        error("workspace utility no longer preserves 642 official indices")
...
@inline paper_release_paper_scale(aux::PaperReleaseAux) =
    aux.trusted.paper_scale
...
function enable_release_runtime!(
    ...
    trusted.paper_scale === true ||
        error("FinalProduction requires a V3 paper_scale=true artifact")
    data = JLD2.load(path)
...
@inline function add_cell_event!(
    ...
    _release_location_is_legal(location) ||
        error("synaptic event targets forbidden soma/axon segment ID $location")
...
function paper_preflight_integrity!(trainer::PaperTrainer)
    ...
    aux.trusted.paper_scale === true ||
        error("production preflight is not paper_scale")
    _assert_release_legal_contact_state!(aux)
...
function paper_checkpoint_integrity!(trainer::PaperTrainer)
    ...
    aux.trusted.paper_scale === true ||
        error("production checkpoint is not paper_scale")
    _assert_release_legal_contact_state!(aux)
...
function paper_end_run_integrity!(trainer::PaperTrainer)
    ...
    aux.trusted.paper_scale === true ||
        error("production run is not paper_scale")
    _assert_release_legal_contact_state!(aux)
