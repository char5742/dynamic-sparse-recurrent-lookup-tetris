"""
Production TwinProp parity binding for corrected official Spieler ELM-v2.

This is the canonical 1,278-channel implementation.  The earlier 3,852-input
two-plane model is retained only as a reconstruction control.
"""
...
@eval TwinPropParityOfficial begin
    ...
    @eval TwinPropParity begin
        ...
        twin_predict(
            ...
            input,
        ) = PaperELMTwinOfficialV2.twin_forward(frozen, input)
        ...
        function receptor_event_tensor(
            ...
            size(excitatory, 1) ==
                ...
                throw(DimensionMismatch(
                    "official parity requires the 642-segment Hay axis",
                ))
            flattened = reshape(spikes, size(spikes, 1), :)
            e_events = excitatory * flattened
            i_events = inhibitory * flattened
            dendrites =
                PaperELMTwinOfficialV2.HAY_FIRST_DENDRITIC_SEGMENT:PaperELMTwinOfficialV2.HAY_LAST_DENDRITIC_SEGMENT
            signed = vcat(
    ...
    official_signed_event_tensor(
        ...
        spikes::AbstractArray{<:Real,3};
        temperature::Real=config.location_temperature_end,
    ) = TwinPropParity.receptor_event_tensor(
        parameters,
        code,
        capacity,
        config,
    ...
    function _official_v2_lineage(
        ...
        _metadata_get(
            ...
            error("official ELM lacks an explicit fidelity pass")
        _metadata_get(
            lineage,
            :frozen_internal,
            _metadata_get(metadata, :frozen_internal, false),
        ) === true ||
            error("official ELM is not certified internally frozen")
        ...
        held_out === nothing &&
            error("official ELM lacks held-out test metrics")
        spike_auroc = Float64(
        ...
        Float64(_metadata_get(
            ...
            error("official ELM declares a weaker AUROC threshold")
    ...
    validate_official_frozen_twin(
        frozen::PaperELMTwinOfficialV2.FrozenOfficialELMTwin,
        catalog::OfficialSegmentCatalog,
    ) = _official_v2_lineage(frozen, catalog)
