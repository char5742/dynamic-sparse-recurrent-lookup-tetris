...
param(
    ...
    [string]$OutputRoot = "D:\tetris-paper-plus\runs\beat_first_v1\sparse_3l_teacher_signal_5000_v1",
...
$contractPath = Join-Path $comparisonRoot "three_layer_5000_update_contract.toml"
...
if (Test-Path -LiteralPath $resolvedOutput) { throw "Signal output root already exists: $resolvedOutput" }
...
try {
    ...
    if ($liveJulia.Count -ne 0) { throw "Refusing to start while Julia PID(s) are live: $(($liveJulia.Id) -join ',')" }
    ...
    $env:BEAT_3L_MAX_UPDATES = "5000"
    $env:BEAT_3L_EVAL_INTERVAL = "500"
    ...
    while (-not $ownedJulia.WaitForExit(100)) {
        if ([DateTimeOffset]::Now -gt $deadline) {
            ...
            throw "Three-layer signal exceeded the frozen fifteen-minute wall-time limit"
        }
    ...
    if ($ownedJulia.ExitCode -ne 0) { throw "Three-layer signal trainer exited with code $($ownedJulia.ExitCode)" }
    [ordered]@{
    ...
    if (Test-Path -LiteralPath $resolvedOutput) {
        [ordered]@{status="failed"; failed_at=[DateTimeOffset]::Now.ToString("o"); error=$_.Exception.Message; output_root=$resolvedOutput} |
