...
function write_partial(path, stage, started, details)
    atomic_write_json(
        path,
        (;
            status="running",
...
function main(args=ARGS)
    length(args) == 4 || error(
        "usage: run_benchmark.jl OUTPUT_DIR SUBSET_NPZ CHECKPOINT FREEZE_JSON"
    )
    output_directory, subset_path, checkpoint_path, freeze_path = abspath.(args)
    ...
    reference_seconds = @elapsed updated_reference = historical_scores(
        model,
        current_parameters,
        fixed_state,
        inputs[1],
    ...
    export_seconds = @elapsed npzwrite(weight_path, arrays)
    npzwrite(
        reference_path,
        Dict(
            "board" => inputs[1][1],
